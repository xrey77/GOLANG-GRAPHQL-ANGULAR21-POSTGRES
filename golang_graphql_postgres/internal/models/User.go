...
type User struct {
	...
	Email       string    `gorm:"size:255;not null;uniqueIndex:idx_email,where:deleted_at IS NULL"`
	Mobile      string    `gorm:"size:255;default:null"`
	Username    string    `gorm:"size:255;not null;uniqueIndex:idx_username,where:deleted_at IS NULL"`
